...
| `PUMPX2_ALLOW_ORACLE_SKIP=1` | **Never set.** Silently drops byte-parity for every oracle-gated suite — today FIVE: `OracleParityTests`, `ResponseParityTests`, `HistoryLogOracleParityTests`, `SignedResponseHmacVerifyTests`, `JpakeInteropTests`. (The long-quoted count of "four" predates `SignedResponseHmacVerifyTests` — an HMAC-verification suite, the worst one to lose silently. Re-derive the list with `grep -rl "enabled(if: OracleRunner.isAvailable)\|enabled(if: JpakeOracle.available)" Tests/` rather than trusting a number here.) CI must not set it; local Swift-only iteration only. |
