...
// TandemLoopKit — a LoopKit `PumpManager` driver for Tandem pumps, built on TandemKit.
//
// This is a SEPARATE, OPTIONAL package. It is intentionally NOT a target of the root
// TandemKit Package.swift, so `swift build` / `swift test` / the byte-exact oracle-parity
// job in the TandemKit root never resolve or compile LoopKit. See README.md.
//
...
let package = Package(
    ...
    platforms: [.iOS(.v16)],
    products: [
        .library(name: "TandemLoopKit", targets: ["TandemLoopKit"]),
    ],
    dependencies: [
        // TandemKit by path — this package lives inside the TandemKit repo. A path (not a
        // version) dependency is REQUIRED: TandemKit's CMbedTLSJPAKE target uses `.unsafeFlags`,
    ...
    targets: [
        .target(
            name: "TandemLoopKit",
            dependencies: [
                .product(name: "TandemMessages", package: "TandemKit"),
                .product(name: "TandemAuth", package: "TandemKit"),
                .product(name: "TandemBLE", package: "TandemKit"),
            ...
            // strict-concurrency conformance against a non-isolated dependency. The core TandemKit
            // targets keep their own Swift 6 mode — this is scoped to the driver.
            swiftSettings: [.swiftLanguageMode(.v5)]
        ),
        .testTarget(
            name: "TandemLoopKitTests",
            dependencies: ["TandemLoopKit"],
            swiftSettings: [.swiftLanguageMode(.v5)]
        ),
    ]
)
