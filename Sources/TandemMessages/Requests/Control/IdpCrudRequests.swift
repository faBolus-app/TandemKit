...
public struct CreateIDPRequest: Message {
    public static let props = MessageProps(
        opCode: 0xE6, size: 35, signed: true, type: .request, characteristic: .control, modifiesInsulinDelivery: true,
        responseOpCode: 0xE7, supportedDevices: [.mobi])
    public var cargo: [UInt8]
    public private(set) var name = ""
    public init() { cargo = [] }
    public init(
        name: String, firstSegmentProfileCarbRatio: UInt32, firstSegmentProfileStartTime: Int = 0,
...
public struct DeleteIDPRequest: Message {
    public static let props = MessageProps(
        opCode: 0xAE, size: 2, signed: true, type: .request, characteristic: .control, modifiesInsulinDelivery: true,
        responseOpCode: 0xAF, supportedDevices: [.mobi])
    public var cargo: [UInt8]
    ...
    public mutating func parse(_ raw: [UInt8]) {
        let b = removeSignedRequestHmacBytes(raw)
        cargo = b
        if b.count >= 2 {
            idpId = Int(b[0])
            profileIndex = Int(b[1])
        }
    }
}
...
public struct RenameIDPRequest: Message {
    public static let props = MessageProps(
        opCode: 0xA8, size: 19, signed: true, type: .request, characteristic: .control, modifiesInsulinDelivery: true,
        responseOpCode: 0xA9, supportedDevices: [.mobi])
    public var cargo: [UInt8]
    public private(set) var idpId = 0, profileIndex = 0
...
public struct SetIDPSegmentRequest: Message {
    // The device-family claim is INHERITED for consistency with sibling IDP-CRUD requests, not
    // independently proven against the upstream annotation (upstream's own @MessageProps carries
    // no MOBI_ONLY tag). It fails SAFE (no-send). Metadata-only: no wire bytes change,
    // OracleParity unaffected.
    public static let props = MessageProps(
        opCode: 0xAA, size: 17, signed: true, type: .request, characteristic: .control, responseOpCode: 0xAB,
        supportedDevices: [.mobi])
    public var cargo: [UInt8]
    public init() { cargo = [] }
    public init(
        idpId: Int, profileIndex: Int, segmentIndex: Int, operationId: Int,
...
public struct SetIDPSettingsRequest: Message {
    ...
    // OracleParity unaffected.
    public static let props = MessageProps(
        opCode: 0xAC, size: 6, signed: true, type: .request, characteristic: .control, responseOpCode: 0xAD,
        supportedDevices: [.mobi])
    public var cargo: [UInt8]
    public init() { cargo = [] }
