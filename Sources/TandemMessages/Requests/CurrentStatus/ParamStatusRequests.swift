...
public struct BolusPermissionChangeReasonRequest: Message {
    public static let props = MessageProps(
        opCode: 0xA8, size: 2, type: .request, characteristic: .currentStatus, responseOpCode: 0xA9, minApi: .future)
    ...
    public mutating func parse(_ raw: [UInt8]) {
        let b = raw.count == Self.props.size ? raw : Bytes.dropFirst(raw, 3)
        cargo = b
        if b.count >= 2 { bolusId = Bytes.readShort(b, 0) }
    }
}
...
public struct CgmSupportPackageStatusRequest: Message {
    public static let props = MessageProps(
        opCode: 0xC8, size: 1, type: .request, characteristic: .currentStatus, responseOpCode: 0xC9, minApi: .future)
    public var cargo: [UInt8]
    public private(set) var deviceType = 0
    public init() { cargo = [] }
    public init(deviceType: Int) {
        self.deviceType = deviceType
        cargo = [UInt8(deviceType & 0xFF)]
    }
    public mutating func parse(_ raw: [UInt8]) {
        let b = raw.count == Self.props.size ? raw : Bytes.dropFirst(raw, 3)
        cargo = b
        if !b.isEmpty { deviceType = Int(b[0]) }
    }
}
...
public struct CommonSoftwareInfoRequest: Message {
    public static let props = MessageProps(
        opCode: 0x8E, size: 1, type: .request, characteristic: .currentStatus, responseOpCode: 0x8F, minApi: .future)
    public var cargo: [UInt8]
    public private(set) var mcuType = 0
    public init() { cargo = [] }
    public init(mcuType: Int) {
        self.mcuType = mcuType
        cargo = [UInt8(mcuType & 0xFF)]
    }
    public mutating func parse(_ raw: [UInt8]) {
        let b = raw.count == Self.props.size ? raw : Bytes.dropFirst(raw, 3)
        cargo = b
        if !b.isEmpty { mcuType = Int(b[0]) }
    }
}
...
public struct CreateHistoryLogRequest: Message {
    public static let props = MessageProps(
        opCode: 0x7E, size: 4, type: .request, characteristic: .currentStatus, responseOpCode: 0x7F,
    ...
    public init() { cargo = [] }
    public init(numberOfLogs: UInt32) {
        self.numberOfLogs = numberOfLogs
        cargo = Bytes.toUint32(numberOfLogs)
    }
    public mutating func parse(_ raw: [UInt8]) {
        let b = raw.count == Self.props.size ? raw : Bytes.dropFirst(raw, 3)
        cargo = b
        if b.count >= 4 { numberOfLogs = Bytes.readUint32(b, 0) }
    }
}
...
public struct StreamDataReadinessRequest: Message {
    public static let props = MessageProps(
        opCode: 0xC6, size: 1, type: .request, characteristic: .currentStatus, responseOpCode: 0xC7,
        minApi: .benchConservativeUnverifiedFloor)  // CONSERVATIVE/UNVERIFIED bench floor (T-1 op-77, >2.5 only)
    public var cargo: [UInt8]
    public private(set) var streamDataType = 0
    public init() { cargo = [] }
    public init(streamDataType: Int) {
        self.streamDataType = streamDataType
        cargo = [UInt8(streamDataType & 0xFF)]
    }
    public mutating func parse(_ raw: [UInt8]) {
        let b = raw.count == Self.props.size ? raw : Bytes.dropFirst(raw, 3)
        cargo = b
        if !b.isEmpty { streamDataType = Int(b[0]) }
    }
}
...
public struct PumpFeaturesV2Request: Message {
    public static let props = MessageProps(
        opCode: 0xA0, size: 1, type: .request, characteristic: .currentStatus, responseOpCode: 0xA1, minApi: .v2_5)
    public var cargo: [UInt8]
